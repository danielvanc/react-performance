...
function appReducer(state, action) {
  switch (action.type) {
    ...
    // We're now using recoil to update the cell 
    // so no extra cases required for cells here
    default: {
...
function AppProvider({children}) {
  const [state, dispatch] = React.useReducer(appReducer, {
    dogName: '',
    // State is now moved outside of React and instead uses Recoil
  })
  // No memoising now needed because we're using Recoil
...
function Grid() {
  // No longer storing the grid in app state, so instead get
  // the updateGrid function from useUpdateGrid
...
function Cell({row, column}) {
  // Use Recoil to update the selected cell
  const [cell, setCell] = useRecoilState(cellAtoms({row, column}))
  // useRecoilState(cellAtoms());
